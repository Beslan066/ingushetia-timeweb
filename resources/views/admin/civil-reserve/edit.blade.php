    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                ...
                <form action="{{route('admin.civilReserves.update', $civilReserve->id)}}" method="post" enctype="multipart/form-data">
                    @csrf
                    @method('patch')
                    <div class="card-body">
                        <div>
                            <div class="form-group w-50">
                                ...
                                <input class="form-control form-control-lg mb-3" type="text" placeholder="Введите заголовок" name="title" value="{{$civilReserve->title}}">
                            </div>
                                    ...
                                    <div class="card">
                                        <div class="card-body">
                                            ...
                                            <input type="file" class="dropify" data-height="300" name="file" multiple
                                                   @if($civilReserve->file)
                                                       data-default-file="{{ asset('storage/' . $civilReserve->file) }}"
                                                @endif
                                            />
                        ...
                        <div class="form-group w-50">
                            ...
                            <select class="form-control" id="exampleFormControlSelect1" name="type">
                                ...
                                @foreach($types as $type)
                                    <option value="{{ $type['id'] }}" {{ $civilReserve->type == $type['id'] ? 'selected' : '' }}>
                                        {{ $type['title'] }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group w-50">
                            <input type="datetime-local" class="datetime_input" name="published_at" style="color: #495057; width: 250px; border: 1px solid #ced4da; padding: 5px !important; "
                                   value="{{ $civilReserve->published_at ? date('Y-m-d\TH:i', strtotime($civilReserve->published_at)) : '' }}">
                        </div>
