      ...
      <div class="card">
        ...
        <div class="card-body">
          <div class="mb-2 d-flex justify-content-between">
            <div>
              <h2>Векторы развития Ингушетии</h2>
            </div>

            <div>
              <a href="{{route('admin.vectors.create')}}">
                  ...
                  @if($vectorsCount !== 0)
                      @foreach($vectors as $vector)
                        <tr role="row" class="odd">
                          @if(isset($vector->id))
                            <td class="" tabindex="0">{{$vector->id}}</td>
                          @endif

                          @if(isset($vector->name))
                            <td class="" tabindex="0">{{mb_substr($vector->name, 0, 80)}}</td>
                          @endif
                          @if(isset($vector->created_at))
                            <td>{{$vector->created_at}}</td>
                          @endif
                          <td style="">
                            <div class="btn-group">
                              @if(isset($vector->id))
                                <a href="{{route('admin.vectors.edit', $vector->id)}}"
                                   class="btn btn-outline-primary waves-effect waves-light">Редактировать</a>
                              @endif

                              @if(isset($vector->id))
                                <form action="{{route('admin.vectors.delete', $vector->id)}}" method="post">
                                  @csrf
                                  @method('delete')
