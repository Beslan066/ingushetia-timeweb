...
@section('content')
  <div class="row">
    <div class="col-12">
      <div class="card">
        <form action="{{ route('admin.vectors.update', $vector->id) }}" method="post" enctype="multipart/form-data">
          @csrf
          @method('patch')
          <div class="card-body">
            ...
            <div class="form-group w-50">
              ...
              <input class="form-control form-control-lg mb-3" type="text"
                     ...
                     value="{{ old('name', $vector->name) }}" required>
              @error('name')
            ...
            <div class="form-group w-50">
              ...
              <textarea class="form-control summernote"
                        ...
                        name="description" id="description">{{ old('description', $vector->description) }}</textarea>
              @error('description')
              <div class="text-danger">{{ $message }}</div>
              @enderror
            </div>
                                        ...
                                        <div class="card-body">
                                            ...
                                            <input type="file" class="dropify" data-height="300" name="image_main" multiple
                                                   @if($vector->image_main)
                                                       data-default-file="{{ asset('storage/' . $vector->image_main) }}"
                                                @endif
                                            />
                                        </div>
                                    </div>
            ...
            <div class="form-group w-50">
              ...
              <select class="form-control" id="exampleFormControlSelect1" name="category_id">
                @if(isset($vector->category->id))
                  <option value="{{$vector->category->id}}">{{$vector->category->title}}</option>
                @endif
            ...
            <div id="sections-container">
              @foreach($vector->sections as $index => $section)
            ...
            <div class="btn-group mt-4">
              <a href="{{ route('admin.vectors.index') }}" class="btn btn-light mr-2">Назад</a>
...
@push('scripts')
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      let sectionCount = {{ count($vector->sections) }};
