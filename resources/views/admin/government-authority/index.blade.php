      ...
      <div class="card">
        ...
        <div class="card-body">
          <div class="mb-2 d-flex justify-content-between">
            <div>
              <h2>Полномочия Правительства</h2>
            </div>

            @if($governmentAuthorityCount < 1)
              <div>
                <a href="{{route('admin.governmentAuthoritys.create')}}">
            ...
            <div class="row">
              <div class="col-sm-12">
                <table id="selection-datatable"
                  ...
                  @if($governmentAuthorityCount !== 0)
                      <tr role="row" class="odd">
                        @if(isset($governmentAuthority->id))
                          <td class="" tabindex="0">{{$governmentAuthority->id}}</td>
                        @endif

                        @if(isset($governmentAuthority->name))
                           <td class="" tabindex="0">{{mb_substr($governmentAuthority->name, 0, 80)}}</td>
                        @endif
                        @if(isset($governmentAuthority->created_at))
                            <td>{{$governmentAuthority->created_at}}</td>
                        @endif
                        <td style="">
                          <div class="btn-group">
                            @if(isset($governmentAuthority->id))
                              <a href="{{route('admin.governmentAuthoritys.edit', $governmentAuthority->id)}}"
                                 class="btn btn-outline-primary waves-effect waves-light">Редактировать</a>
                            @endif

                            @if(isset($governmentAuthority->id))
                                <form action="{{route('admin.governmentAuthoritys.delete', $governmentAuthority->id)}}" method="post">
                                  @csrf
                                  @method('delete')
